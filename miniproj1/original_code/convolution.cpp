...
void convolution_layer_simple(
  VTYPE (&synapse)[Ky][Kx][Nn][Ni], 
  VTYPE (&neuron_i)[NYPAD][NXPAD][Ni], 
  VTYPE (&neuron_n)[Ny][Nx][Nn]) 
{
    ...
    for (int n = 0; n < Nn; n++) {
      ...
      for (int ky = 0; ky < Ky; ky++) {
        for (int kx = 0; kx < Kx; kx++) {
          ...
          for (int i = 0; i < Ni; i++) {
            VTYPE weight = synapse[ky][kx][n][i];
            VTYPE input_val = neuron_i[y + ky][x + kx][i];
...
int main(const int argc, const char** argv) {
  ...
  convolution_layer_simple(*synapse,*neuron_i,*neuron_n2);
  end_roi();
